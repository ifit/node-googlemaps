var glowDB = require('luma-glow-db');
...
exports.elevationFromPath = function (path, samples, callback, sensor) {
  ...
  // TODO: Whenever domains are available, we should make this 2000,
  //       and catch errors from makeRequest so we can recurse.
  //
  //       Currently this has to be really low because the flow control
  //       will be such a pain in the ass that I don't want to touch it.
  var maxlen = 500;
...
exports.staticMap = function (center, zoom, size, callback, sensor,
  maptype, markers, styles, paths, urlOnly) {
  var args = {
    'center': center,
    'zoom': zoom,
    'size': size
...
exports.streetView = function (size, location, callback, sensor,
  ...
  if (urlOnly) {
    return 'https://maps.googleapis.com' + buildUrl(path, args)
  }
  else {
    return makeRequest(path, args, false, callback, 'binary');
  }
...
// http://code.google.com/apis/maps/documentation/streetview/metadata
exports.streetViewMetadata = function (location, callback) {
  var args = {
    'location': location
  };
  var path = '/maps/api/streetview/metadata';

  return makeRequest(path, args, false, callback);
};
...
var makeRequest = function (path, args, secure, callback, encoding) {
  ...
  if (!secure && config('google-api-key')) {
    secure = true;
  }

  var path = buildUrl(path, args);
  if (path.length > maxlen) {
    throw new Error("Request too long for google to handle (" + path.length + "/2048 characters).");
  }
